...
namespace proxy {
 ...
 using response_serializer =
  http::response_serializer<buffer_body, http::fields>;
 using string_response_serializer =
  http::response_serializer<string_body, http::fields>;
...
<hr><center>nginx/1.20.2</center>
 ...
 // global_known_proto 用于指定全局已知的协议,
 // 用于噪声注入时避免生成已知的协议头.
 ...
 class proxy_session : public proxy_session_base,
  ...
  template <typename Stream> tcp::socket& net_tcp_socket(Stream& socket) {
 ...
 public:
  proxy_session(net::any_io_executor executor, variant_stream_type&& socket,
   size_t id, std::weak_ptr<proxy_server_base> server,
   bool tproxy = false)
   : m_executor(executor), m_local_socket(std::move(socket)),
   m_remote_socket(init_proxy_stream(executor)), m_udp_socket(executor),
   m_timer(executor), m_connection_id(id), m_tproxy(tproxy),
 ...
 public:
  void start() override {
   ...
   if (!m_option.proxy_pass_.empty()) {
    try {
     ...
     log_conn_error() << ", params next_proxy error: "
      << m_option.proxy_pass_ << ", exception: " << e.what();
  ...
  void set_tproxy_remote(const net::ip::tcp::endpoint& tproxy_remote) override {
   m_tproxy_remote = tproxy_remote;
  }

  size_t connection_id() override { return m_connection_id; }

 private:
  inline net::awaitable<void> transparent_proxy() {
   ...
   bool ret = co_await connect_bridge_proxy(
    remote_socket, m_tproxy_remote.address().to_string(),
   ...
   log_conn_debug() << ", transfer completed"
    << ", local to remote: " << l2r_transferred
    << ", remote to local: " << r2l_transferred;

   co_return;
  }

  inline net::awaitable<bool> noise_handshake(tcp::socket& socket,
   std::vector<uint8_t>& inkey,
   std::vector<uint8_t>& outkey) {
   boost::system::error_code error;

   // 工作流程:
   // 1. 生成一段随机长度的随机数据(最大长度由配置文件中的参数 noise_length
   // 指定), 用于发送给对方.
   // 2. 根据这些随机数据计算发送数据的 key, 这个 key
   // 将会用于后续的代理时数据的加密.
   // 3. 发送随机数据.
   // 4. 对方在接收到随机数据后, 同样会发送噪声随机数据(包含随机数长度本身,
   // 在前16字节中的最后一位,
   //    组成一个 16 位的整数表示长度).
   ...
   log_conn_debug() << ", send noise, length: " << noise.size();

   // 发送 noise 消息.
   co_await net::async_write(socket, net::buffer(noise), net_awaitable[error]);
   if (error) {
    log_conn_warning() << ", noise write error: " << error.message();
   ...
   // 接收对方发过来的 noise 回应消息.
   co_await net::async_read(socket, net::buffer(noise, 16),
    net_awaitable[error]);

   if (error) {
    log_conn_warning() << ", noise read header error: " << error.message();
   ...
   if (remainder < 0 || remainder >= std::numeric_limits<uint16_t>::max()) {
    log_conn_debug() << ", noise length: " << noise_length
   ...
   co_await net::async_read(socket, net::buffer(noise.data() + 16, remainder),
    net_awaitable[error]);

   if (error) {
    log_conn_warning() << ", noise read body error: " << error.message();

    co_return false;
   }

   log_conn_debug() << ", recv noise, length: " << noise.size();
  ...
  inline net::awaitable<void> proto_detect(bool handshake_before = true) {
   ...
   co_await socket.async_wait(net::socket_base::wait_read,
    net_awaitable[error]);
   if (error) {
   ...
   // handshake_before 在调用 proto_detect 时第1次为 true, 第2次调用
   // proto_detect 时 handshake_before 为 false, 此时就表示已经完成了 scramble
   // 握手并协 商好了 scramble 加解密用的 key, 则此时应该为 socket
   // 配置好加解密用的 key.
   ...
   if (m_option.disable_insecure_) {
    ...
    if ((proto_byte != 0x05 && proto_byte != 0x04 && proto_byte != 0x47 &&
     proto_byte != 0x50 && proto_byte != 0x43) ||
     !handshake_before) {
     noise_proto = true;
    }
   ...
   else if (detect[0] == 0x16) // http/socks proxy with ssl crypto protocol.
   {
    ...
    auto ssl_socks_stream =
     init_proxy_stream(std::move(socket), srv_ssl_context);
    ...
    co_await ssl_socket.async_handshake(net::ssl::stream_base::server,
     net_awaitable[error]);

    if (error) {
     log_conn_debug() << ", ssl server protocol handshake error: "
      << error.message();
     co_return;
    }
    ...
    detect[0] == 0x43)   // 'C'
   {
    ...
    if (!co_await noise_handshake(net_tcp_socket(socket), m_inin_key,
     m_inout_key))
     co_return;
    ...
    log_conn_debug() << ", unknown protocol: 0x" << std::hex
     << (int)proto_byte;
   }
  ...
  inline net::awaitable<void> start_proxy() {
   ...
   [[maybe_unused]] auto bytes =
    co_await net::async_read(m_local_socket, m_local_buffer,
     net::transfer_exactly(2), net_awaitable[ec]);
   if (ec) {
    log_conn_error() << ", read socks version: " << ec.message();
    co_return;
   }
   ...
   if (socks_version == SOCKS_VERSION_5) {
    if (m_option.disable_socks_) {
     log_conn_debug() << ", socks5 protocol disabled";
     co_return;
    }

    log_conn_debug() << ", socks version: " << socks_version;
   ...
   if (socks_version == SOCKS_VERSION_4) {
    if (m_option.disable_socks_) {
     log_conn_debug() << ", socks4 protocol disabled";
     co_return;
    }
   ...
   if (socks_version == 'G' || socks_version == 'P') {
    ...
    if (!ret) {
     auto fake_page = fmt::vformat(
      fake_400_content_fmt, fmt::make_format_args(server_date_string()));

     co_await net::async_write(m_local_socket, net::buffer(fake_page),
      net::transfer_all(), net_awaitable[ec]);
    }
   }
  ...
  inline net::awaitable<void> socks_connect_v5() {
   ...
   if (nmethods <= 0 || nmethods > 255) {
    log_conn_error() << ", unsupported method : " << nmethods;
    co_return;
   }
   ...
   boost::system::error_code ec;
   auto bytes = co_await net::async_read(m_local_socket, m_local_buffer,
    net::transfer_exactly(nmethods),
    net_awaitable[ec]);
   if (ec) {
    log_conn_error() << ", read socks methods: " << ec.message();
    co_return;
   }
   ...
   bytes = co_await net::async_write(
    m_local_socket, wbuf, net::transfer_exactly(2), net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", write server method error : " << ec.message();
    co_return;
   }

   if (method == SOCKS5_AUTH_UNACCEPTABLE) {
    log_conn_warning() << ", no acceptable methods for server";
    co_return;
   }
   ...
   bytes =
    co_await net::async_read(m_local_socket, m_local_buffer,
     net::transfer_exactly(5), net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", read client request error: " << ec.message();
    co_return;
   }
   ...
   if (ver != SOCKS_VERSION_5) {
    log_conn_warning() << ", socks requests, invalid protocol: " << ver;
    co_return;
   }
   ...
   bytes = co_await net::async_read(m_local_socket, m_local_buffer,
    net::transfer_exactly(length),
    net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", read client request dst.addr error: "
     << ec.message();
    co_return;
   }
   ...
   if (atyp == SOCKS5_ATYP_IPV4) {
    ...
    log_conn_debug() << ", " << m_local_socket.remote_endpoint()
    ...
    do {
     ...
     if (atyp == SOCKS5_ATYP_DOMAINNAME) {
      ...
      auto targets = co_await resolver.async_resolve(
       domain, std::to_string(port), net_awaitable[ec]);
      if (ec)
       break;
     ...
     log_conn_debug() << ", local udp address: "
     ...
     wbuf.commit(len);
     bytes = co_await net::async_write(m_local_socket, wbuf,
   ...
   // 连接成功或失败.
    {
     ...
     wbuf.commit(len);
     bytes = co_await net::async_write(
      m_local_socket, wbuf, net::transfer_exactly(len), net_awaitable[ec]);
     if (ec) {
      log_conn_warning() << ", write server response error: " << ec.message();
      co_return;
     }
    ...
    log_conn_debug() << ", connected start transfer";

    // 发起数据传输协程 - SOCKS5专用透明传输
    if (command == SOCKS_CMD_CONNECT) {
     size_t l2r_transferred = 0;
     size_t r2l_transferred = 0;
  ...
  inline net::awaitable<void> forward_udp() {
   ...
   while (!m_abort) {
    ...
    auto bytes = co_await m_udp_socket.async_receive_from(
     net::buffer(wbuf, 4000), remote_endp, net_awaitable[ec]);
    if (ec)
     break;
    ...
    if (remote_endp.address() == m_local_udp_address) {
     ...
     co_await m_udp_socket.async_send_to(net::buffer(rp, udp_size),
      remote_endp, net_awaitable[ec]);
    }
    else // 如果数据包来自远程主机, 则解析数据包并将数据转发给 socks 客户端.
    {
     ...
     co_await m_udp_socket.async_send_to(net::buffer(wbuf, udp_size),
      local_endp, net_awaitable[ec]);
    }
   }

   log_conn_debug() << ", recv total: " << recv_total
  ...
  inline net::awaitable<void> tick() {
   ...
   while (!m_abort) {
    ...
    if (ec) {
     log_conn_warning() << ", ec: " << ec.message();
     break;
    }

    if (--m_udp_timeout <= 0) {
     log_conn_debug() << ", udp socket expired";
     m_udp_socket.close(ec);
     break;
    }
   }

   log_conn_debug() << ", udp expired timer quit";
  ...
  inline net::awaitable<void> socks_connect_v4() {
   ...
   auto bytes =
    co_await net::async_read(m_local_socket, m_local_buffer,
     net::transfer_exactly(6), net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", read socks4 dst: " << ec.message();
    co_return;
   }
   ...
   //                                          [                   ]
   m_local_buffer.consume(m_local_buffer.size());
   bytes = co_await net::async_read_until(m_local_socket, m_local_buffer, '\0',
    net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", read socks4 userid: " << ec.message();
    co_return;
   }
   ...
   std::string hostname;
   if (socks4a) {
    bytes = co_await net::async_read_until(m_local_socket, m_local_buffer,
     '\0', net_awaitable[ec]);
    if (ec) {
     log_conn_warning() << ", read socks4a hostname: " << ec.message();
     co_return;
    }
   ...
   log_conn_debug() << ", use " << (socks4a ? "domain: " : "ip: ")
    << (socks4a ? hostname
   ...
   if (verify_passed)
    log_conn_debug() << ", auth passed";
   else
    log_conn_warning() << ", auth no pass";
   ...
   if (!verify_passed) {
    ...
    wbuf.commit(8);
    bytes = co_await net::async_write(
     m_local_socket, wbuf, net::transfer_exactly(8), net_awaitable[ec]);
    if (ec) {
     log_conn_warning() << ", write socks4 no allow: " << ec.message();
     co_return;
    }

    log_conn_warning() << ", socks4 " << userid << " auth fail";
    co_return;
   }
   ...
   if (command == SOCKS_CMD_CONNECT) {
    if (socks4a)
     ...
     co_await start_connect_host(dst_endpoint.address().to_string(), port,
      ec);
    if (ec) {
     XLOG_FWARN("connection id: {},"
      " connect to target {}:{} error: {}",
      m_connection_id, dst_endpoint.address().to_string(), port,
      ec.message());
   ...
   bytes = co_await net::async_write(
    m_local_socket, wbuf, net::transfer_exactly(8), net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", write socks4 response: " << ec.message();
    co_return;
   }
  ...
  inline int http_authorization(std::string_view pa) {
   ...
   std::string userinfo(beast::detail::base64::decoded_size(auth.size()), 0);
   auto [len, _] = beast::detail::base64::decode((char*)userinfo.data(),
    auth.data(), auth.size());
   userinfo.resize(len);
  ...
  inline net::awaitable<bool> http_proxy_get() {
   ...
   while (!m_abort) {
    ...
    // 读取 http 请求头.
    co_await http::async_read(m_local_socket, m_local_buffer, *parser,
     net_awaitable[ec]);
    if (ec) {
     log_conn_warning() << (keep_alive ? ", keepalive" : "")
    ...
    log_conn_debug() << ", method: " << mth << ", target: " << target_view
     << (pa.empty() ? std::string()
    ...
    if (auth != PROXY_AUTH_SUCCESS || !get_url_proxy) {
     ...
     if (!expect_url.has_error()) {
      log_conn_warning() << ", proxy err: " << pauth_error_message(auth);
    ...
    const auto scheme_id =
     urls::string_to_scheme(target_view.substr(0, authority_pos - 3));
    ...
    for (auto pos = authority_pos; pos < target_view.size(); pos++) {
     ...
     if (c == '@') {
      ...
      if (port_start != std::string::npos)
       port = (uint16_t)std::atoi(
        target_view.substr(port_start, pos - port_start).c_str());
    ...
    if (ec) {
     log_conn_warning() << ", http_proxy_get request async_write: "
      << ec.message();
     co_return !first;
    }
    ...
    auto bytes = co_await http::async_read(m_remote_socket, buf, parser,
     net_awaitable[ec]);
    if (ec) {
     log_conn_warning() << ", http_proxy_get response async_read: "
      << ec.message();
     co_return !first;
    }

    co_await http::async_write(m_local_socket, parser.release(),
     net_awaitable[ec]);
    if (ec) {
     log_conn_warning() << ", http_proxy_get response async_write: "
      << ec.message();
     co_return !first;
    }
  ...
  inline net::awaitable<bool> http_proxy_connect() {
   ...
   // 读取 http 请求头.
   co_await http::async_read(m_local_socket, m_local_buffer, req,
    net_awaitable[ec]);
   if (ec) {
    log_conn_error() << ", http_proxy_connect async_read: " << ec.message();
   ...
   if (auth != PROXY_AUTH_SUCCESS) {
    ...
    auto fake_page = fmt::vformat(
     fake_407_content_fmt, fmt::make_format_args(server_date_string()));
   ...
   if (pos == std::string::npos) {
    log_conn_error() << ", illegal target: " << target_view;
    co_return false;
   }
   ...
   co_await start_connect_host(
    host, static_cast<uint16_t>(std::atol(port.c_str())), ec, true);
   if (ec) {
    XLOG_FWARN("connection id: {},"
     " connect to target {}:{} error: {}",
     m_connection_id, host, port, ec.message());
  ...
  inline net::awaitable<bool> socks_auth() {
   ...
   auto bytes =
    co_await net::async_read(m_local_socket, m_local_buffer,
     net::transfer_exactly(2), net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", read client username/passwd error: "
     << ec.message();
    co_return false;
   }
   ...
   if (auth_version != 1) {
    log_conn_warning() << ", socks negotiation, unsupported socks5 protocol";
    co_return false;
   }
   int name_length = read<uint8_t>(p);
   if (name_length <= 0 || name_length > 255) {
    log_conn_warning() << ", socks negotiation, invalid name length";
    co_return false;
   }
   name_length += 1;
   ...
   bytes = co_await net::async_read(m_local_socket, m_local_buffer,
    ...
    net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", read client username error: " << ec.message();
    co_return false;
   }
   ...
   if (passwd_len <= 0 || passwd_len > 255) {
    log_conn_warning() << ", socks negotiation, invalid passwd length";
    co_return false;
   }
   ...
   bytes = co_await net::async_read(m_local_socket, m_local_buffer,
    ...
    net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", read client passwd error: " << ec.message();
    co_return false;
   }
   ...
   log_conn_debug() << ", auth: " << uname << ", passwd: " << passwd
    << ", client: " << client;
   ...
   //  +----+--------+
   wbuf.commit(2);
   co_await net::async_write(m_local_socket, wbuf, net::transfer_exactly(2),
    net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", server write status error: " << ec.message();
    co_return false;
   }
  ...
  net::awaitable<void> transfer(S1& from, S2& to, size_t& bytes_transferred) {
   ...
   // 完全透明转发：直接使用原生TCP socket，绕过所有proxy_stream包装
   tcp::socket& from_sock = net_tcp_socket(from);
   tcp::socket& to_sock = net_tcp_socket(to);

   // 使用8KB缓冲区，这是很多应用的默认缓冲区大小
   std::array<char, 8192> buffer;
   boost::system::error_code ec;

   // 完全原生的socket转发循环
   while (!m_abort) {
    // 直接使用原生socket的async_read_some，保持Chromium的读取特征
    auto bytes = co_await from_sock.async_read_some(
    ...
    // 直接使用原生socket的async_write，保持Chromium的写入特征
    co_await net::async_write(to_sock, net::buffer(buffer.data(), bytes),
     net_awaitable[ec]);
   ...
   // 静默关闭连接
   boost::system::error_code ignore_ec;
   to_sock.shutdown(net::socket_base::shutdown_send, ignore_ec);
  }

  template <typename Stream, typename Endpoint>
  inline bool check_condition(const boost::system::error_code&, Stream& stream,
   Endpoint&) const {
   if (!m_bind_interface)
    return true;
  ...
  inline net::awaitable<bool>
   connect_bridge_proxy(tcp::socket& remote_socket, std::string target_host,
   ...
   if (m_bridge_proxy->port_number() == 0)
    proxy_port =
    std::to_string(urls::default_port(m_bridge_proxy->scheme_id()));
   else
    proxy_port = std::to_string(m_bridge_proxy->port_number());
   if (proxy_port.empty())
    proxy_port = m_bridge_proxy->scheme();

   log_conn_debug() << ", connect to next proxy: " << proxy_host << ":"
    << proxy_port;
   ...
   if (!detect_hostname(proxy_host)) {
    ...
    targets = tcp::resolver::results_type::create(endp, proxy_host,
     m_bridge_proxy->scheme());
   }
   else {
    targets = co_await resolver.async_resolve(proxy_host, proxy_port,
     net_awaitable[ec]);

    if (ec) {
     XLOG_FWARN("connection id: {},"
      " resolver to next proxy {}:{} error: {}",
      m_connection_id, std::string(m_bridge_proxy->host()),
      std::string(m_bridge_proxy->port()), ec.message());
   ...
   if (m_option.scramble_) {
    ...
    log_conn_debug() << ", with upstream noise completed";
   }
   ...
   auto instantiate_stream =
    [this, &scheme, &proxy_host, &remote_socket,
    ...
    if (m_option.proxy_pass_use_ssl_ || scheme == "https") {
     ...
     // 生成 ssl socket 对象.
     auto sock_stream =
      init_proxy_stream(std::move(remote_socket), m_ssl_cli_context);
     ...
     // do async handshake.
     co_await ssl_socket.async_handshake(net::ssl::stream_base::client,
      net_awaitable[ec]);
     if (ec) {
      XLOG_FWARN("connection id: {},"
   ...
   log_conn_debug() << ", connect to next proxy: " << proxy_host << ":"
    << proxy_port << " start upstream handshake with "
    << std::string(scheme);
   ...
   if (scheme.starts_with("socks")) {
    ...
    co_await async_http_proxy_handshake(m_remote_socket, opt,
     net_awaitable[ec]);
   }
   ...
   if (ec) {
    XLOG_FWARN("connection id: {}"
     ...
     m_connection_id, std::string(scheme), target_host, target_port,
     ec.message());
  ...
  inline net::awaitable<bool> start_connect_host(std::string target_host,
   ...
   if (m_bridge_proxy) {
    auto ret = co_await connect_bridge_proxy(remote_socket, target_host,
     target_port, ec);
    ...
    if (resolve) {
     ...
     if (ec) {
      log_conn_warning()
       << ", resolve: " << target_host << ", error: " << ec.message();
     ...
     dst_endpoint.address(net::ip::make_address(target_host));
     dst_endpoint.port(target_port);

     targets =
      net::ip::basic_resolver_results<tcp>::create(dst_endpoint, "", "");
    }
    ...
    if (ec) {
     XLOG_FWARN("connection id: {}, connect to target {}:{} error: {}",
      m_connection_id, target_host, target_port, ec.message());

     co_return false;
    }

    // 完全不设置任何TCP选项，保持目标连接的原始特征
    // 让Chromium的流量特征完全透明传输
  ...
  inline net::awaitable<void>
   normal_web_server(http::request<http::string_body>& req,
    std::optional<request_parser>& parser) {
   boost::system::error_code ec;
   ...
   for (; !m_abort;) {
    if (!has_read_header) {
     ...
     co_await http::async_read_header(m_local_socket, m_local_buffer,
      *parser, net_awaitable[ec]);
     if (ec) {
      log_conn_debug() << (keep_alive ? ", keepalive" : "")
    ...
    if (req[http::field::expect] == "100-continue") {
     ...
     co_await http::async_write(m_local_socket, res, net_awaitable[ec]);
     if (ec) {
      log_conn_debug() << ", web expect async_write: " << ec.message();
     }
     co_return;
    }
    ...
    if (beast::websocket::is_upgrade(req)) {
     auto fake_page = fmt::vformat(
      fake_404_content_fmt, fmt::make_format_args(server_date_string()));
    ...
    http_context http_ctx{
        {}, req, target, make_real_target_path(req.target()) };

#define BEGIN_HTTP_ROUTE()                                                     \
  if (false) {                                                                 \
  }
...
#define END_HTTP_ROUTE()                                                       \
    ...
    co_await default_http_route(req, fake_400_content,                         \
  ...
  inline fs::path path_cat(const std::wstring& doc,
...
#ifdef WIN32
   slash = L"\\";
   if (doc.back() == L'/' || doc.back() == L'\\')
    slash = L"";
   return fs::path(doc + slash + std::wstring(sv));
#else
   if (doc.back() == L'/')
    slash = L"";
   return fs::path(boost::nowide::narrow(doc + slash + std::wstring(sv)));
#endif // WIN32
  };

  template <typename Path> inline std::string make_unc_path(const Path& path) {
  ...
  inline std::wstring make_target_path(const std::string& target) {
   try {
    auto result = urls::url(target.starts_with("/") ? target : "/" + target);
  ...
  inline std::tuple<std::string, fs::path>
   ...
   static auto loc_time = [](auto t) -> struct tm* {
    ...
    if constexpr (std::is_same_v<time_type,
     ...
     static_assert(!std::is_same_v<time_type, time_type>,
      "time type required!");
    }
    };
  ...
  inline std::vector<std::wstring>
   ...
   if (ec) {
    log_conn_debug() << ", format_path_list read dir: " << path
     << ", error: " << ec.message();
    return {};
   }
   ...
   for (; it != end && !m_abort; it++) {
    ...
    if (fs::is_directory(unc_path.empty() ? item : unc_path, ec)) {
     ...
     auto str =
      fmt::format(body_fmt, rpath, show_path, space, time_string, L"-");
     ...
     if (ec)
      sz = 0;
     filesize = boost::nowide::widen(strutil::add_suffix(sz));
     auto show_path = rpath;
     if (show_path.size() > 50) {
      show_path = show_path.substr(0, 47);
      show_path += L"..&gt;";
  ...
  inline std::string file_hash(const fs::path& p,
   ...
   std::unique_ptr<char, decltype(&std::free)> bufs(
  ...
  inline auto async_hash_file(const fs::path& path, CompletionToken&& token) {
   ...
   return net::async_initiate<CompletionToken,
    void(boost::system::error_code, std::string)>(
     [this, self, path](auto&& handler) mutable {
      std::thread([this, self, path,
       handler = std::move(handler)]() mutable {
        ...
        net::post(executor,
         [this, self, ec = std::move(ec), hash = std::move(hash),
         handler = std::move(handler)]() mutable {
          handler(ec, hash);
         });
       }).detach();
  ...
  inline net::awaitable<void> on_http_all_json(const http_context& hctx) {
   ...
   auto target =
    fs::path(make_real_target_path(hctx.command_[0])).lexically_normal();
   ...
   if (ec) {
    ...
    if (ec) {
     log_conn_warning() << ", http_dir write location err: " << ec.message();
    }
   ...
   co_await http::async_write(m_local_socket, sr, net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", http dir write body err: " << ec.message();
   }
  ...
  inline net::awaitable<void> on_http_dir(const http_context& hctx) {
   ...
   if (fs::exists(index_html, ec)) {
    ...
    if (file) {
     ...
     co_await http::async_write(m_local_socket, sr, net_awaitable[ec]);
     if (ec) {
      log_conn_warning() << ", http dir write index err: " << ec.message();
     }
   ...
   std::wstring head = fmt::format(head_fmt, target_path, target_path);

   std::wstring body = fmt::format(body_fmt, L"../", L"../", L"", L"", L"");
  ...
  inline net::awaitable<void> on_http_get(const http_context& hctx) {
   ...
   if (!fs::exists(path, ec)) {
    log_conn_warning() << ", http " << hctx.target_ << " file not exists";
   ...
   if (fs::is_directory(path, ec)) {
    log_conn_debug() << ", http " << hctx.target_ << " is directory";
    ...
    try {
     ...
     log_conn_warning() << ", http " << hctx.target_
      << " location error: " << e.what();
    }
   ...
   if (ec) {
    ...
    co_await default_http_route(request, fake_400_content,
     http::status::bad_request);

    co_return;
   }

   std::fstream file(path.string(), std::ios_base::binary | std::ios_base::in);
   ...
   log_conn_debug()
    << ", http file: " << hctx.target_ << ", size: " << content_length
    << (request.count("Range") ? ", range: " + std::string(request["Range"])
     : std::string())
   ...
   if (range.size() == 1) {
    ...
    if (r.first == -1) {
     ...
     if (r.second < 0) {
      co_await default_http_route(request, fake_416_content,
   ...
   if (st == http::status::partial_content) {
    ...
    std::string content_range =
     fmt::format("bytes {}-{}/{}", r.first, r.second, content_length);
   ...
   if (ec) {
    log_conn_warning() << ", http async_write_header: " << ec.message();
   ...
   do {
    auto bytes_transferred =
     fileop::read(file, std::span<char>(buf, buf_size));
    bytes_transferred =
     std::min<std::streamsize>(bytes_transferred, content_length - total);
    ...
    // 不设置超时，保持原始行为
    // stream_expires_after(m_local_socket,
    //                     std::chrono::seconds(m_option.tcp_timeout_));
    ...
    if (ec) {
     log_conn_warning() << ", http async_write: " << ec.message()
      << ", already write: " << total;
     co_return;
    }
   } while (!sr.is_done());
   ...
   XLOG_DBG << "connection id: " << m_connection_id
    << ", http request: " << hctx.target_ << ", completed";
  ...
  inline std::string server_date_string() {
   ...
   std::string str(64, '\0');
   auto ret =
    strftime((char*)str.data(), 64, "%a, %d %b %Y %H:%M:%S GMT", gmt);
   str.resize(ret);

   return str;
  }

  inline net::awaitable<void> default_http_route(const string_request& request,
   std::string response,
   http::status status) {
   boost::system::error_code ec;
   ...
   co_await http::async_write(m_local_socket, sr, net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", default http route err: " << ec.message();
   }

   co_return;
  }

  inline net::awaitable<void> location_http_route(const string_request& request,
   ...
   co_await http::async_write(m_local_socket, sr, net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", location http route err: " << ec.message();
   }
  ...
  inline net::awaitable<void>
   forbidden_http_route(const string_request& request) {
   boost::system::error_code ec;
   ...
   co_await http::async_write(m_local_socket, sr, net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", forbidden http route err: " << ec.message();
   }
  ...
  inline net::awaitable<void>
   unauthorized_http_route(const string_request& request) {
   boost::system::error_code ec;
   ...
   co_await http::async_write(m_local_socket, sr, net_awaitable[ec]);
   if (ec) {
    log_conn_warning() << ", unauthorized http route err: " << ec.message();
   }
  ...
  inline void stream_expires_after(variant_stream_type& stream,
   net::steady_timer::duration expiry_time) {
   // 完全禁用超时设置，保持原始socket行为
   // if (expiry_time.count() < 0)
   //   return;

   // boost::variant2::visit(
   //     [expiry_time](auto &s) mutable {
   //       using ValueType = std::decay_t<decltype(s)>;
   //       using NextLayerType = util::proxy_tcp_socket::next_layer_type;
  ...
  inline void stream_expires_at(variant_stream_type& stream,
 ...
 private:
  ...
  // m_timer 用于定时检查 udp 会话是否过期, 由于 udp 通信是无连接的, 如果 2
  // 端长时间 没有数据通信, 则可能会话已经失效, 此时应该关闭 udp socket
  // 以及相关资源.
 ...
 class proxy_server : public proxy_server_base,
  ...
  proxy_server(net::any_io_executor executor, proxy_server_option opt)
   : m_executor(executor), m_option(std::move(opt)), m_timer(executor) {
 ...
 public:
  inline static std::shared_ptr<proxy_server>
   ...
   return std::shared_ptr<proxy_server>(new proxy_server(executor, opt));
  }

  virtual ~proxy_server() = default;

  inline bool rfc2818_verification_match_pattern(const char* pattern,
  ...
  inline void
   walk_certificate(const fs::path& directory,
    std::vector<certificate_file>& certificates) noexcept {
   ...
   if (file.cert_.type_ != pem_type::none &&
    ...
    // 如果设置了 ssl_prefer_server_ciphers_ 则设置
    ...
    if (ec) {
     XLOG_WARN << "use_certificate_chain_file: " << file.cert_.filepath_
      << ", error: " << ec.message();
     return;
    }

    // 设置 password 文件, 如果存在的话.
    if (file.pwd_.type_ != pem_type::none &&
     fs::exists(file.pwd_.filepath_)) {
    ...
    // 设置私钥文件.
    ssl_ctx.use_private_key_file(file.key_.filepath_.string(),
     net::ssl::context::pem, ec);
    if (ec) {
     XLOG_WARN << "use_private_key_file: " << file.key_.filepath_
      << ", error: " << ec.message();
     return;
    }

    // 设置 dhparam 文件, 如果存在的话.
    if (file.dhparam_.type_ != pem_type::none &&
     fs::exists(file.dhparam_.filepath_)) {
     ssl_ctx.use_tmp_dh_file(file.dhparam_.filepath_.string(), ec);
     if (ec) {
      XLOG_WARN << "use_tmp_dh_file: " << file.dhparam_.filepath_
       << ", error: " << ec.message();
      return;
     }
    }
    ...
    std::unique_ptr<GENERAL_NAMES, decltype(&GENERAL_NAMES_free)>
     general_names{ static_cast<GENERAL_NAMES*>(X509_get_ext_d2i(
                       x509_cert, NID_subject_alt_name, 0, 0)),
                   &GENERAL_NAMES_free };
    ...
    for (int i = 0; i < sk_GENERAL_NAME_num(general_names.get()); i++) {
     ...
     if (gen->type == GEN_DNS) {
      ...
      if (domain->type == V_ASN1_IA5STRING && domain->data &&
       domain->length) {
       file.alt_names_.emplace_back((const char*)(domain->data),
        domain->length);
      }
     }
    }
    ...
    X509_NAME_get_text_by_NID(X509_get_subject_name(x509_cert),
     NID_commonName, cert_cname, sizeof cert_cname);
  ...
  inline void
   update_certificate(const fs::path& directory,
    std::vector<certificate_file>& certificates) noexcept {
   ...
   for (const auto& ctx : certificates) {
    XLOG_DBG << "domain: '" << ctx.domain_ << "', expire: '"
     << ctx.expire_date_ << "', cert: '"
     << ctx.cert_.filepath_.string() << "', key: '"
     << ctx.key_.filepath_.string() << "', dhparam: '"
     << ctx.dhparam_.filepath_.string() << "', pwd: '"
  ...
  inline void init_ssl_context() noexcept {
   ...
   // 设置 SNI 回调函数.
   SSL_CTX_set_tlsext_servername_callback(m_ssl_srv_context.native_handle(),
    proxy_server::ssl_sni_callback);
   ...
   while (!m_abort) {
    ...
    for (const auto& ctx : certificates) {
     if (now > ctx.expire_date_) {
      XLOG_WARN << "domain: '" << ctx.domain_ << "', cert: '"
       << ctx.cert_.filepath_.string() << "', key: '"
       << ctx.key_.filepath_.string() << "', dhparam: '"
       << ctx.dhparam_.filepath_.string() << "', pwd: '"
       << ctx.pwd_.filepath_.string() << "', expired: '"
       << ctx.expire_date_ << "'";
      continue;
     }

     duration =
      std::chrono::seconds((ctx.expire_date_ - now).total_seconds());
     break;
    }
...
#define IPV6_TRANSPARENT 75
    ...
    // 设置 acceptor 为透明代理模式.
    using transparent =
     net::detail::socket_option::boolean<IPPROTO_IP, IP_TRANSPARENT>;
    using transparent6 =
     net::detail::socket_option::boolean<IPPROTO_IPV6, IPV6_TRANSPARENT>;
   ...
   // 启动 证书检查定时器.
   net::co_spawn(m_executor, certificate_check_timer(), net::detached);
  }
 ...
 private:
  void remove_session(size_t id) override { m_clients.erase(id); }

  size_t num_session() override { return m_clients.size(); }

  const proxy_server_option& option() override { return m_option; }

  net::ssl::context& ssl_context() override { return m_ssl_srv_context; }
 ...
 private:
  ...
  // 的对象.
  inline net::awaitable<void>
   start_proxy_listen(tcp_acceptor& acceptor) noexcept {
   boost::system::error_code error;

   // 完全不设置任何TCP选项，保持系统默认值
   ...
   while (!m_abort) {
    ...
    co_await acceptor.async_accept(socket.lowest_layer(),
     net_awaitable[error]);
    if (error) {
     if (!m_abort)
    ...
    // 完全不修改任何TCP选项，保持操作系统默认设置
    // 这样可以确保Chromium等浏览器的流量特征完全不被改变

    // 创建 proxy_session 对象.
    auto new_session = std::make_shared<proxy_session>(
     m_executor, init_proxy_stream(std::move(socket)), connection_id,
     self);
  ...
  inline net::awaitable<bool>
   start_transparent_proxy(proxy_tcp_socket& socket,
    size_t connection_id) noexcept {
...
#define SO_ORIGINAL_DST 80
   ...
   if (::getsockopt(sockfd, protocol, SO_ORIGINAL_DST, (char*)&addr,
    &addrlen) < 0) {
   ...
   auto it = std::find_if(m_local_addrs.begin(), m_local_addrs.end(),
    [&](const auto& addr) {
   ...
   if (it == m_local_addrs.end()) {
    XLOG_DBG << "connection id: " << connection_id
     << ", is tproxy, remote: " << remote_endp;

    auto self = shared_from_this();

    // 创建 proxy_session 对象用于 tproxy.
    auto new_session = std::make_shared<proxy_session>(
     m_executor, init_proxy_stream(std::move(socket)), connection_id, self,
     true);
  ...
  inline net::awaitable<void> get_local_address() noexcept {
   ...
   auto results =
    co_await resolver.async_resolve(hostname, "", net_awaitable[ec]);
   if (ec) {
  ...
  inline bool ip_filter(const std::string& ip_cidr,
   ...
   try {
    ...
    if (!ec) {
     auto target =
      net::ip::make_network_v4(ipaddr.to_v4(), netaddr4.netmask());
     if (target == netaddr4)
      return true;
     return false;
    }
    ...
    if (!ec) {
     auto target =
      net::ip::make_network_v6(ipaddr.to_v6(), netaddr6.prefix_length());
     if (target == netaddr6)
      return true;
     return false;
    }
  ...
  inline bool
   region_filter(const std::vector<std::string>& local_info) const noexcept {
 ...
 private:
  ...
  using proxy_session_weak_ptr = std::weak_ptr<proxy_session>;
