...
bool Util::is_space(const char c)
{
	if (c == ' ' ||
		c == '\f' ||
		c == '\n' ||
...
std::string_view Util::string_trim(std::string_view sv)
{
...
bool Util::parse_endpoint_string(std::string_view str,
	std::string& host, std::string& port, bool& ipv6only)
{
	ipv6only = false;

	auto address_string = Util::string_trim(str);
